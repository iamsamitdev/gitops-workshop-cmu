...
> ⚠️ **ต้องใช้ Classic PAT เท่านั้น!**  
> Fine-grained PAT ไม่มี `packages` permission — GitHub ยังไม่รองรับ

ไปที่ GitHub → **Settings → Developer settings → Personal access tokens → **Tokens (classic)****

**ตั้งค่า:**
- **Note:** `GHCR_TOKEN`
- **Expiration:** 90 days (หรือตามที่ต้องการ)
- **Select scopes:**
  - ☑️ `write:packages` ← สำคัญที่สุด (push image ไป GHCR)
  - ☑️ `read:packages` ← (ติดมาอัตโนมัติ)
...
kubectl create secret generic argocd-repo-manifest-repo \
  --namespace=argocd \
...
kubectl label secret argocd-repo-manifest-repo \
  -n argocd \
